...
import { Volume2, CheckCircle2, XCircle, Lightbulb, Loader2, BookA, Quote } from 'lucide-react';
...
export const VocabularyModule = ({ course, module }: VocabularyModuleProps) => {
  ...
  const backMeaning = primaryDefinition
    ? `${primaryDefinition.definition}${currentDetail!.meanings[0].partOfSpeech ? ` (${currentDetail!.meanings[0].partOfSpeech})` : ''}`
    : currentWord.meaning;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-center">
        <div
          className="flip-card w-96 h-64 cursor-pointer"
          onClick={handleFlip}
        >
          <div className={`flip-card-inner w-full h-full relative transition-transform duration-500 ${
            ...
            <div className="flip-card-back absolute inset-0 bg-white rounded-2xl flex flex-col items-center justify-center p-6 border-2 border-primary-200 overflow-y-auto">
              <div className="flex items-center gap-2 mb-1">
                <h4 className="text-xl font-bold text-gray-800">{currentWord.word}</h4>
                {backPhonetic && (
              ...
              {loadingDetail ? (
                ...
                <p className="text-base text-primary-600 font-medium mb-3 text-center">
                  {backMeaning}
                </p>
              )}
